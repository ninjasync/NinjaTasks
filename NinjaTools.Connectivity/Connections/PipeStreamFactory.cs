...
{
    /// <summary>
    /// For testing network functionality it is often desireable to tests the data exchange 
    ...
    /// </summary>
    public class PipeStreamFactory: IStreamListener, IStreamConnector, IStreamFactory
    {
        ...
        public PipeStreamFactory(int bufferSize = 1024, string name = null)
        {
            BufferSize = bufferSize;
            ServiceName = name ?? "PipeStreams";
            Log = LogManager.GetLogger(typeof (PipeStreamFactory).FullName + (name == null ? "" : "." + name));
        }
        ...
        public async Task<Stream> ConnectAsync()
        {
            ...
            await _waitConnect.WaitAsync().ConfigureAwait(false);
        ...
        public async Task<Stream> ListenAsync(CancellationToken token)
        {
            ...
            Stream pWeWrite = new MonitorStream(new PipeStream(BufferSize), true);
            Stream pWeRead = new MonitorStream(new PipeStream(BufferSize), true);
        ...
        IStreamConnector IStreamFactory.GetConnector(RemoteDeviceInfo device)
        {
            return this;
        }
