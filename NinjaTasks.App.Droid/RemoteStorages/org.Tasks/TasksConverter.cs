...
namespace NinjaTasks.App.Droid.RemoteStorages.org.Tasks
{
    ...
    class TasksConverter
    {
        public OrgTasksTodoTask TodoTaskFromCursor(ICursor cursor)
        {
            ...
            ret.SetId(cursor.GetLong(TasksContract.ColId), cursor.GetString(TasksContract.ColTaskRemoteId));
            ret.Description = cursor.GetString(TasksContract.ColTaskTitle);
            
            ret.CreatedAt = cursor.GetDateTimeFromUnixMillies(TasksContract.ColTaskCreated);

            ret.ModifiedAt = cursor.GetDateTimeFromUnixMillies(TasksContract.ColTaskModified);
            if (ret.ModifiedAt == default(DateTime)) 
                ret.ModifiedAt = ret.CreatedAt;

            ret.CompletedAt = cursor.GetDateTimeFromUnixMillies(TasksContract.ColTaskCompleted);
            if (ret.CompletedAt == default(DateTime)) 
                ret.CompletedAt = null;

            ret.Status = ret.CompletedAt != null ? Status.Completed : Status.Pending;

            TasksContract.Importance imp = (TasksContract.Importance) cursor.GetInt(TasksContract.ColTaskImportance);
        ...
        public OrgTasksTagsTagMetadata MetadataFromCursor(ICursor cursor)
        {
            ...
            ret.Id = cursor.GetInt(TasksContract.ColId);
            ret.TaskId = cursor.GetInt(TasksContract.ColMetadataTask);
            ret.TagName = cursor.GetString(TasksContract.ColMetadataValue);
            ret.TagUuid = cursor.GetString(TasksContract.ColMetadataValue2);
            ret.TaskUuid = cursor.GetString(TasksContract.ColMetadataValue3);
            return ret;
        }
