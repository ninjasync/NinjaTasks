...
namespace NinjaTasks.App.Droid.RemoteStorages
{
    public static class ContentResolverExtensions
    {
        ...
        /// </summary>
        public static ICursor QueryByIds(this ContentResolver resolver, Uri uri, string[] columns, string colId,
            long[] ids)
        {
            if (ids.Length == 0)
        ...
        /// if ids is empty, returns all rows.
        /// </summary>
        public static ICursor QueryByIds(this ContentProviderClient provider, Uri uri, string[] columns, string colId,
            long[] ids)
        {
            if (ids.Length == 0)
            ...
            string[] ids)
        {
            ...
            string query = string.Format("{0} in ({1})", colId,
                string.Join(",", ids.Select(DatabaseUtils.SqlEscapeString)));
            return provider.Query(uri, columns, query, null, null);
        }
    }
}