...
namespace NinjaTasks.App.Droid.RemoteStorages.NonsenseApps
{
    public class NpConverter
    {
        ...
        public TodoTask TodoTaskFromCursor(ICursor cursor)
        {
            ...
            ret.Id = CursorExtensions.GetLong(cursor, NpContract.ColId).ToStringInvariant();
            ret.Description = CursorExtensions.GetString(cursor, NpContract.ColTaskTitle);
            ret.ListFk = CursorExtensions.GetLong(cursor, NpContract.ColTaskDblist).ToStringInvariant();
            
            ret.CompletedAt = CursorExtensions.GetDateTimeFromUnixMilliesNullable(cursor, NpContract.ColTaskCompleted);
            if (ret.CompletedAt == default(DateTime)) ret.CompletedAt = null;

            ret.ModifiedAt = CursorExtensions.GetDateTimeFromUnixMillies(cursor, NpContract.ColTaskUpdated);

            if (ret.ModifiedAt == default (DateTime))
                ret.ModifiedAt = ret.CreatedAt;

            ret.SortPosition = CursorExtensions.GetInt(cursor, NpContract.ColTaskLeft);
        ...
        public TodoList TodoListFromCursor(ICursor cursor)
        {
            ...
            ret.Description = CursorExtensions.GetString(cursor, NpContract.ColListTitle);
            ret.ModifiedAt = CursorExtensions.GetDateTimeFromUnixMillies(cursor, NpContract.ColListUpdated);
