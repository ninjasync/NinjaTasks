...
namespace NinjaTools.IPC.Console
{
    ...
    /// <summary>
    /// class to easy communicate with Console Applications
    /// 
    ...
    public class ConsoleProcess : IConsoleProcess
    {
        ...
        public event Action<string, CommunicationSource> LineRead;

        public event Action<byte[], int> StderrDataRead;
        public event Action<byte[], int> StdoutDataRead;
        public event Action<byte[], int, CommunicationSource> DataRead;
        ...
        private void SetupStreams()
        {
            ...
            {
                _stderrLineReader = new AsyncLineReader(_stderrReader, Encoding.Default);
            ...
            {
                _stdoutLineReader = new AsyncLineReader(_stdoutReader, Encoding.Default);
        ...
        private void _stdoutReader_DataRead(byte[] data, int len)
        {
            ...
            if (ModeStdout == CommunicationMode.Binary && DataRead != null)
                DataRead(data, len, CommunicationSource.Stdout);
        }
        ...
        private void _stderrReader_DataRead(byte[] data, int len)
        {
            ...
            if (ModeStderr == CommunicationMode.Binary && DataRead != null)
                DataRead(data, len, CommunicationSource.Stderr);
        ...
        private void _stdoutLineReader_LineRead(string line)
        {
            ...
            if (ModeStdout == CommunicationMode.Line && StdoutLineRead != null)
                StdoutLineRead(line);
            if (ModeStdout == CommunicationMode.Line && LineRead != null)
                LineRead(line, CommunicationSource.Stdout);
        }
        ...
        private void _stderrLineReader_LineRead(string line)
        {
            ...
            if (ModeStderr == CommunicationMode.Line && StderrLineRead != null)
                StderrLineRead(line);
            if (ModeStderr == CommunicationMode.Line && LineRead != null)
                LineRead(line, CommunicationSource.Stderr);
        }
        ...
        public void Kill(int waitBeforeKillMs, bool waitAsyncronous = true)
        {
            if (!IsRunning) return;
            try
            {
        ...
        public void StdinWrite(string msg)
        {
            if (Encoding == null)
            {
